...
import { ActionTypeIcon, CUSTOM_ICON_OPTIONS } from "@/components/action-icons";
...
export default function ActionCard({ action, landingId, update, remove, move, first, last }: Props) {
  ...
  const [icon, setIcon] = useState(CUSTOM_ICON_OPTIONS.some((option) => option.id === action.icon) ? action.icon : "");
  ...
  return <article className="action-card">
    ...
    <form action={update} className="stack compact-form">
      ...
      {needsUrl && <label className="label">URL<input name="url" type="text" defaultValue={action.url || ""} placeholder="tusitio.com" required /></label>}
