...
export function parseLogoStyle(raw: unknown): LogoStyle {
  ...
  return { ...value, treatment: treatments.includes(value.treatment) ? value.treatment : "template", shape: shapes.includes(value.shape) ? value.shape : "round", backgroundMode: value.backgroundMode === "custom" ? "custom" : "auto", fallback: /^#[0-9a-f]{6}$/i.test(value.fallback) ? value.fallback : DEFAULT_LOGO_STYLE.fallback, borderWidth: (() => { const w = Math.min(10, Math.max(0, finite(value.borderWidth, 0))); return w === 1 ? 2 : w; })(), borderColor: /^#[0-9a-f]{6}$/i.test(value.borderColor) ? value.borderColor : "#ffffff", // Landings saved before shadowColor existed have no value for it, and back then glow/hard
    // shadows were drawn in borderColor — so falling back to borderColor (not to the default
    // black) keeps every already-published page rendering exactly as it did.
    shadowColor: /^#[0-9a-f]{6}$/i.test(value.shadowColor) ? value.shadowColor : (/^#[0-9a-f]{6}$/i.test(value.borderColor) ? value.borderColor : DEFAULT_LOGO_STYLE.shadowColor), shadow: shadows.includes(value.shadow) ? value.shadow : "soft", shadowSize: Math.min(2, Math.max(.5, finite(value.shadowSize, 1))), initials: initialsModes.includes(value.initials) ? value.initials : "one", size: Math.min(190, Math.max(72, finite(value.size, 124))), zoom: Math.min(2.5, Math.max(1, finite(value.zoom, 1))), x: Math.min(100, Math.max(0, finite(value.x, 50))), y: Math.min(100, Math.max(0, finite(value.y, 50))) };
}
